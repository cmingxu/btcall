...
class User < ActiveRecord::Base
  ...
  validates :mobile, format: { with: /\d{11}/, message: "你输入的手机号码不正确", :allow_blank => true }
  ...
  has_one :latest_sms_notice,  -> { where("status = 'sent_out'").order("id DESC").limit(1) } ,:class_name => "SmsNotice"
